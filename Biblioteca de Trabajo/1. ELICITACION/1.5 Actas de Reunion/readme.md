# Actas de Reunión

Este directorio contiene los documentos que forman parte del Elemento de Configuración del Software (ECS) relacionado con las actas de reunión del proyecto.

## Información del ECS

- **Nombre del ECS:** Actas de Reunión
- **Responsables:** Fernando Sandoval, Tipán Oswaldo, Simone Medina, Zaith Mandangón
- **Proyecto:** [Nombre de tu proyecto]
- **Línea base:** LBR - Línea Base Requisitos
- **Localización:** Biblioteca de Trabajo/1. ELICITACION/1.5 Actas de Reunion
- **Tipo de ECS:** Documento (PDF, DOCX)
- **Fecha de creación:** 04/12/2025
- **ID del proyecto:** 27835_G3_ADS

## Historial de Versiones de las Actas de Reunión

| Versión       | Fecha      | Responsable   | Aprobado por                                               |
| ------------- | ---------- | ------------- | ---------------------------------------------------------- |
| AR_V1.0.0.pdf | 04/12/2025 | Tipán Oswaldo | Aprobado por Tipán Oswaldo, Simone Medina, Zaith Mandangón |

Estas versiones reflejan la evolución y formalización de las actas de reunión, aprobadas para el desarrollo del sistema.
